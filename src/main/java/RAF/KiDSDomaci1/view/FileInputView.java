...
public class FileInputView {
    ...
    public FileInputView(FileInput fileInput) {
        this.fileInput = fileInput;
        fileInputWorker = new FileInputWorker(fileInput);
        initView();
        initThreads();
    }
    ...
    private void initThreads() {
        ...
        Thread workerThread = new Thread(fileInputWorker);
        thread.start();
        workerThread.start();
    }
    ...
    private void initView() {
        main = new VBox();
        main.getChildren().add(new Text("File input: " + fileInput.getDisk().toString()));
        ...
        status.textProperty().bind(fileInputWorker.messageProperty());
    ...
    private void addDirectory() {
        ...
        File fileDirectory = directoryChooser.showDialog(MainView.getInstance().getStage());
        if (fileDirectory != null && fileDirectory.exists() && fileDirectory.isDirectory()) {
            ...
            fileInput.getDirectories().add(directory);
        }
    }
    ...
    private void start() {
//        if(fileInput.getWorking().get()) fileInput.getWorking().compareAndSet(true, false);
//        else fileInput.getWorking().compareAndSet(false, true);
//        fileInput.getWorkingLock().notifyAll();

        if (fileInput.getWorking().get()) {
            synchronized (fileInput.getWorkingLock()) {
                if (fileInput.getWorking().compareAndSet(true, false)) {
                    fileInput.getWorkingLock().notify();
                }
            }
            start.setText("Start");
        } else {
            synchronized (fileInput.getWorkingLock()) {
                if (fileInput.getWorking().compareAndSet(false, true)) {
                    fileInput.getWorkingLock().notify();
                }
            }
            start.setText("Pause");
    ...
    private void removeDiskInput() {
        MainView.getInstance().removeFileInputView(this);
    }
